...
@section('content')
    <div class="container p-3 mt-4">
        <div class="d-flex my-3">
            ...
            <form class="d-flex ms-auto" action="/admin/transaksi/lelang" method="get">
                <div class="my-2">
                    <select name="status" class="form-select" style="width: 350px;">
                        <option selected value="">Semua Status</option>
                        <option value="aktif"
                            @isset($status) {{ $status == 'aktif' ? 'selected' : '' }} @endisset>Aktif
                        </option>
                        <option value="non-aktif"
                            @isset($status) {{ $status == 'non-aktif' ? 'selected' : '' }} @endisset>
                            Non-Aktif</option>
                    </select>
                </div>
                <input class="form-control m-2" type="search" name="keyword" placeholder="Nama barang... 🔍"
            ...
            @foreach ($products as $product)
                ...
                <div class="col">
                    <div class="card h-100">
                        ...
                        <div class="card-body">
                            <a href="/admin/transaksi/lelang/{{ $product->id }}" class="btn btn-success w-100">
                                Detail Barang
                            </a>
                        </div>
                    </div>
